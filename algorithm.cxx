              ...
              int count, MPI_Datatype datatype, MPI_Status *status)
{
  ...
  //If I have not been assigned a new bridge node, so write  
   else {
              ...
              int count, MPI_Datatype datatype, MPI_Request *request)
{
  ...
  MPI_Request *req, sendreq, *wrequest; 
  MPI_Status sendst, stat, status, *wstatus; 
  ...
  if (bridgeNodeInfo[1] > 1) {
   ...
   //If I have not been assigned a new bridge node, write 
    ...
    // write own data
    result = PMPI_File_iwrite_at (fh, offset, buf, count, datatype, &wrequest[myWeight]);
    ...
    for (int i=0; i<myWeight ; i++) {
      MPI_Waitany (myWeight, req, &idx, &stat);
      result = PMPI_File_iwrite_at (fh, (MPI_Offset)shuffledNodes[idx]*count*sizeof(double), shuffledNodesData[idx], count, datatype, &wrequest[idx]);
      if (result != MPI_SUCCESS) 
